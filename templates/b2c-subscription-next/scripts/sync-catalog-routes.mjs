...
function findManifestFiles() {
  if (!fs.existsSync(catalogRoot)) {
    return { files: [], reason: 'missing-catalog' };
  }

  const rootManifest = path.join(catalogRoot, 'routes.manifest.json');
  if (fs.existsSync(rootManifest)) return { files: [rootManifest], reason: 'found-root-manifest' };

  const modulesDir = path.join(catalogRoot, 'modules');
  if (!fs.existsSync(modulesDir)) return { files: [], reason: 'missing-modules-dir' };
  ...
  if (manifestFiles.length === 0) {
    return { files: [], reason: 'missing-manifest' };
  }
  return { files: manifestFiles, reason: 'found-module-manifests' };
}
...
const manifestLookup = findManifestFiles();
const manifestFiles = manifestLookup.files;

if (manifestFiles.length === 0) {
  if (manifestLookup.reason === 'missing-catalog') {
    console.log('[routes:sync] skipped: modules/catalog is missing. Connect module-catalog submodule first.');
  } else {
    console.log('[routes:sync] skipped: no route manifests found. Expected modules/catalog/routes.manifest.json or modules/catalog/modules/*/routes.manifest.json');
  }
  console.log('[routes:sync] policy: no-op when catalog/manifest is absent to keep fresh template repositories runnable.');
  process.exit(0);
}
